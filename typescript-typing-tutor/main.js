...
$span = document.querySelectorAll('#phrase-container span');
if (!$span.length)
    throw new Error('No spans were created.');
...
document.addEventListener('keydown', (event) => {
    ...
    if (pressedKey === expectedChar) {
        ...
        if (currentIndex < phrase.length) {
            ...
            if (resultsBox) {
                resultsBox.textContent = `Completed in ${timeTaken.toFixed(2)}s - Speed: ${wordsPerMinute.toFixed(2)} WPM`;
            }
